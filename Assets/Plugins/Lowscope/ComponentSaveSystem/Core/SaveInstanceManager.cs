...
using Plugins.Lowscope.ComponentSaveSystem.Data;
using Plugins.Lowscope.ComponentSaveSystem.Enums;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.SceneManagement;

namespace Plugins.Lowscope.ComponentSaveSystem.Core
{
    /// <summary>
    ...
    public class SaveInstanceManager : MonoBehaviour
    {
        ...
        public void OnLoad(SaveGame saveGame)
        {
            ...
            RemoveUnwantedWorldClutter(saveGame, saveData);
            RestoreMissingFarmTreeSpawnInfo(saveGame, saveData);
        ...
        private void RemoveUnwantedWorldClutter(SaveGame saveGame, SaveData saveData)
        {
            const string plantPrefabId = "72d8fc1f-7ce5-431e-8841-0e083eaef56e";
            const string rockPrefabId = "f8179ff3-ce36-4550-a407-41ba34eba382";

            int removedCount = 0;
            if (saveData != null && saveData.infoCollection != null)
            {
                for (int i = saveData.infoCollection.Count - 1; i >= 0; i--)
                {
                    string prefabId = saveData.infoCollection[i].filePath;
                    if (prefabId == plantPrefabId || prefabId == rockPrefabId)
                    {
                        saveData.infoCollection.RemoveAt(i);
                        removedCount++;
                    }
                }
            }

            SaveGameJSON jsonSaveGame = saveGame as SaveGameJSON;
            if (jsonSaveGame != null)
            {
                for (int i = 0; i < jsonSaveGame.saveData.Count; i++)
                {
                    SaveGameJSON.Data savedData = jsonSaveGame.saveData[i];
                    if (savedData.scene != SceneID || string.IsNullOrEmpty(savedData.guid))
                    {
                        continue;
                    }

                    if (savedData.guid.Contains("-Plant(Clone)-") ||
                        savedData.guid.Contains("-Rock(Clone)-"))
                    {
                        saveGame.Remove(savedData.guid);
                    }
                }
            }
            ...
            {
                changesMade++;
                Debug.LogFormat("Removed {0} unwanted Plant/Rock instance records from {1}.",
                    removedCount, SceneID);
            }
        }

        // Compatibility repair for farm trees whose instance records were removed while
        // the Tree prefab temporarily had no Saveable component. Their Mover records still
        // contain the original instance IDs and positions, so rebuilding SpawnInfo is safe.
        private void RestoreMissingFarmTreeSpawnInfo(SaveGame saveGame, SaveData saveData)
        {
            const string sceneName = "Level_Farm";
            const string instancePrefix = "Level_Farm-Tree(Clone)-";
            const string moverSuffix = "-Mover-a5a1a";
            const string treePrefabId = "a8d6b85d-406e-4b5c-a538-a5486b6abc10";
            const string customSource = "ScriptableAssetDatabase";

            if (SceneID != sceneName || saveData == null || saveData.infoCollection == null)
            {
                return;
            }
            ...
            for (int i = 0; i < saveData.infoCollection.Count; i++)
            {
                existingIds.Add(saveData.infoCollection[i].saveIdentification);
            }
            ...
            {
                SaveGameJSON.Data savedData = jsonSaveGame.saveData[i];
                string id = savedData.guid;

                if (savedData.scene != sceneName || string.IsNullOrEmpty(id) ||
                    !id.StartsWith(instancePrefix) || !id.EndsWith(moverSuffix))
                {
                    continue;
                }

                string instanceId = id.Substring(0, id.Length - moverSuffix.Length);
                if (!existingIds.Add(instanceId))
                {
                    continue;
                }

                saveData.infoCollection.Add(new SpawnInfo
                {
                    source = InstanceSource.Custom,
                    filePath = treePrefabId,
                    saveIdentification = instanceId,
            ...
            {
                changesMade++;
                Debug.LogFormat("Restored {0} missing farm tree instance records.", restoredCount);
            }
        }

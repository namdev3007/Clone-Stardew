...
namespace Referencing.Scriptable_Assets.Editor
{
    ...
    public class ReferenceableAssetPostProcessor
    {
        ...
        private static void LoadItems(bool testingInEditor)
        {
            ...
            bool canCreateAssetDatabase = testingInEditor || (buildIndex == 0);
            ...
            if (canCreateAssetDatabase)
            {
                ...
                for (int i = 0; i < itemGuidPaths.Length; i++)
                {
                    ...
                    if (AssetDatabase.LoadAssetAtPath<ScriptableObject>(path) is IReferenceableAsset getItemData)
                    {
                        string guid = getItemData.GetGuid();
